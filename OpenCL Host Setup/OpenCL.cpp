...
//	Setup_buffers(&CL,&BUF);									//Setup buffers and pass them as a kernel argument


	//Read only Buffer, Input to Kernel
	// No need to write data to kernel buffer coz - "CL_MEM_USE_HOST_PTR"

	BUF.input_buffer = clCreateBuffer(CL.context,CL_MEM_READ_ONLY|CL_MEM_USE_HOST_PTR,sizeof(arr),arr,&err);
	if(err!=CL_SUCCESS)
	{
	...
	BUF.Output_buffer = clCreateBuffer(CL.context,CL_MEM_READ_WRITE,sizeof(int)*10,NULL,&err);
	if(err!=CL_SUCCESS)
	{
	...
	err = clEnqueueReadBuffer(CL.queue,BUF.Output_buffer,TRUE,0,sizeof(int)*10,arr2,NULL,NULL,NULL);
	if(err!=CL_SUCCESS)
	{
